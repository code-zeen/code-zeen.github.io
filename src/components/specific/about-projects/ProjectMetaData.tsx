...
export default function ProjectMetaData({ project }: { project: Project }) {
  ...
  return (
    <div className="flex items-center gap-3">
      <div
        className={`flex justify-center items-center h-7 w-7 shrink-0 rounded-full ${projectColorMap[project.type] || 'bg-gray-400'}`}>
        <Icon name={project.type} />
      </div>
      <div>
        <span className="capitalize">{project.location.text[language]}</span>
